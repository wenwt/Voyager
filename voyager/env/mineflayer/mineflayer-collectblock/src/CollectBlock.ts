                    ...
                    if (options.ignoreNoPath) {
                        ...
                        if (err.name === "Invalid block") {
                            ...
                            if (closest.constructor.name === "Block") {
                                const block = closest as Block;
                                const properties =bot.registry.blocksByName[block.name];
                                if (properties?.harvestTools) {
                                    // 只有当 harvestTools 存在时才执行这里的代码
                                    const leastTool = Object.keys(properties.harvestTools)[0];
                                    const toolId = parseInt(leastTool);
                                    if (!isNaN(toolId)) {
                                        const item = bot.registry.items[toolId];
                                        if (item) {
                                            bot.chat(`I need at least a ${item.name} to mine ${block.name}!  Skip it!`);
                                            return;
                                        }
                                    }
                                }
